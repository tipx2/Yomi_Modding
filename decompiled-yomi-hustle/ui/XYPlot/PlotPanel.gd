...
func mouse_in_bounds():
	return not (mpos.x < 0 or mpos.x > rect_size.x or mpos.y < 0 or mpos.y > rect_size.y)
...
func update_value(p = null):
	...
	$"%XLabel".text = str(values.x) if not parent.normalize_display else str(values.x / float(parent.range_))
	$"%YLabel".text = str(values.y) if not parent.normalize_display else str(values.y / float(parent.range_))
	update()
...
func _draw():
	...
	if parent.limit_angle:
		...
		if not parent.limit_symmetrical:
