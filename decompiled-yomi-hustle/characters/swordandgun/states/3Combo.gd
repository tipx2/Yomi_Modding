...
func _frame_1():
	if not down:
		host.reset_momentum()
		var force = fixed.normalized_vec_times("0.5" if not down else "0.85", "-1.0" if not down else "1.0", "4.0")
		host.apply_force_relative(force.x, force.y)

func _frame_6():
	host.reset_momentum()
	var force = fixed.normalized_vec_times("0.5" if not down else "0.85", "-1.0" if not down else "1.0", "8.0")
	host.apply_force_relative(force.x, force.y)
	if down:
...
func _frame_10():
	host.reset_momentum()
	var force = fixed.normalized_vec_times("0.5" if not down else "0.85", "-1.0" if not down else "1.0", "10.0")
	host.apply_force_relative(force.x, force.y)

func _frame_19():
	host.reset_momentum()
	var force = fixed.normalized_vec_times("0.5" if not down else "0.85", "-1.0" if not down else "1.0", "12.0")
	host.apply_force_relative(force.x, force.y)
