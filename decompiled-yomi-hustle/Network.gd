...
func rpc_(function_name:String, arg = null, type = "remotesync"):
	...
	if direct_connect:
		...
		if not steam and not (multiplayer_client and multiplayer_client.connected):
				return 
		if type == "remote":
			if steam:
...
func host_game_relay(new_player_name, public = true):
	if not (multiplayer_client and multiplayer_client.connected):
		return 
...
remotesync func player_forfeit(player_id):
	if is_instance_valid(game):
		...
		forfeiter = player_id
		if steam and not SteamLobby.SPECTATING:
...
remotesync func register_player(new_player_name, id, version):
	if not ((version is String and Global.VERSION is String) or (version is Dictionary and Global.VERSION is Dictionary)):
